...
ccimage_l = create_clean_image_from_fits_file(os.path.join(uvdata_path,
                                                           images[0]))
ccimage_h = create_clean_image_from_fits_file(os.path.join(uvdata_path,
                                                           images[1]))
shift = ccimage_h.cross_correlate(ccimage_l, region1=(256, 256, 50, None),
                                  region2=(256, 256, 50, None))
...
for i_boot in range(n_boot):
    cc_fits_fnames_glob = 'simul_*_{}_cc.fits'.format(i_boot)
    cc_fits_fnames = glob.glob(os.path.join(data_dir, cc_fits_fnames_glob))
    print "Caculating shifts for bootstrap realization {}".format(i_boot)
    cc_fits_fnames = sorted(cc_fits_fnames)
