...
def suggest_component_from_dirty_residuals(residual_image, comp_type="cg",
                                           min_width_mas=None, max_width_mas=None,
    ...
    if max_width_mas is not None:
        if bmaj > max_width_mas:
            bmaj = max_width_mas
    if min_width_mas is not None:
        if bmaj < min_width_mas:
            bmaj = min_width_mas
    rms = mad_std(image_convolved)
    if blc_2plot is None or trc_2plot is None:
        blc_2plot, trc_2plot = find_bbox(image_convolved, 3*rms, min_maxintensity_jyperbeam=30*rms,
...
def automodeller_v2(basename, uvfits_file, working_dir, mapsize, n_max, comp_type, freq_ghz, path_to_script,
    ...
    blc, trc = find_bbox(original_image.image, 3*original_rms,
                         min_maxintensity_jyperbeam=10*original_rms,
                         min_area_pix=1*100, delta=10)
    ...
    for i in range(n_max):
        ...
        comp = suggest_component_from_dirty_residuals(residuals, min_width_mas=0.25, max_width_mas=2.5,
        ...
        rchisq = modelfit_difmap(uvfits_file, 'pre_fitted_{}.mdl'.format(counter+1),
                                 ...
                                 out_path=working_dir, dmap_name=f"residuals_{counter+1}.fits", niter=200,
...
if __name__ == "__main__":
    ...
    if band == "Q":
        ...
        raise Exception("Band must be u, Q or W!")

    uvfits_dir = f"/home/ilya/Downloads/{source}/{band}"

    save_dir = os.path.join(uvfits_dir, "modelfit_results_cg")
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    print(f"Will put results in {save_dir}")

    uvfits_files = glob.glob(os.path.join(uvfits_dir, "*.uvf"))
    ...
    if check_frequency_in_files:
        for uvfits_file in uvfits_files:
            ...
            if band == "Q":
                ...
                assert np.floor(freq_ghz) == 15.0

    for uvfits_file in uvfits_files[::-1]:
        fn = os.path.split(uvfits_file)[-1]
        epoch = fn.split(".")[2]
        base = f"{source}.{band}"
